  ...
  try {
    ...
    replaceRows_("Sessions", (data.sessions || []).map(function(row) {
      return [
        ...
        row.group || "",
        row.campus || "",
        number_(row.enrolled),
        number_(row.capacity),
...
function clearAll_(user) {
  ["Rooms", "Lecturers", "StudentGroups", "Modules", "Requirements", "Sessions", "Conflicts"].forEach(function(name) {
    replaceRows_(name, []);
  });
...
function askGemini_(question, context) {
  ...
  const faqText = getFaqs_().map(function(item) {
    return "Q: " + item.question + "\nA: " + item.answer;
  }).join("\n\n");
  const contextText = JSON.stringify(context || {}).slice(0, 35000);
  const prompt = [
    "You are the Campus Timetable Intelligence assistant.",
    "Help users understand navigation, data imports, timetable filters, room booking, schedule changes, conflicts, reports and scheduling decisions.",
  ...
  appendAudit_("ASK_GEMINI", "assistant", "", "Timetable Platform", question.slice(0, 500), "Success");
  return answer || "No answer was returned.";
}

function getFaqs_() {
  return readObjects_("FAQs")
    .filter(function(row) { return boolean_(row.active); })
    .map(function(row) {
...
function setConfigValue_(key, value, notes) {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1, 1, 1, SHEET_HEADERS.Config.length).setValues([SHEET_HEADERS.Config]);
  }
...
function appendAudit_(action, entityType, entityId, user, details, status) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, SHEET_HEADERS.AuditLog.length).setValues([SHEET_HEADERS.AuditLog]);
    }
