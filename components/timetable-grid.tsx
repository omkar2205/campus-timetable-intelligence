...
export function TimetableGrid({
  ...
  return <div className="enterprise-card overflow-auto">
    <div className="min-w-[1040px] grid grid-cols-[90px_repeat(5,1fr)]">
      ...
      {teachingDays.map(day => <div key={day} className="border-b border-l border-slate-200 bg-slate-50 p-3">
        <p className="text-sm font-bold text-navy">{day}</p>
        <p className="mt-0.5 text-xs text-slate-500">{formatDayDate(dates[day])}</p>
      </div>)}
      ...
      {teachingTimes.map(time => <Fragment key={time}>
        ...
        {teachingDays.map(day => {
          ...
          return <div
            ...
            className="min-h-36 border-b border-l border-slate-200 p-2 transition hover:bg-teal-50/30"
          >
            <div className="space-y-2">
              {items.map(session => <SessionCard
                key={session.id}
                session={session}
...
function SessionCard({
  ...
  const conflict = Boolean(session.conflict);
  ...
  return <div
    ...
    onClick={onSelect}
    className={`${className} ${movable ? "cursor-grab active:cursor-grabbing" : "cursor-pointer"}`}
  >
    <div className="flex items-start justify-between gap-2">
      <div>
        <p className="font-bold text-navy">{session.moduleCode}</p>
        <p className="mt-1 text-sm font-medium text-slate-700">{session.moduleName}</p>
      </div>
      <div className="flex items-center gap-1">
        {conflict && <StatusBadge value="Critical"/>}
        {movable && <GripVertical className="text-slate-400" size={16}/>} 
      </div>
    </div>
